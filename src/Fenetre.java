...
public class Fenetre extends JFrame implements ActionListener {
	...
	private JTextField fieldTole3a = new JTextField();// essaye
	...
	private JLabel labelWelcome = new JLabel(); //Création du titre principal
	private JLabel labelTol3a = new JLabel();
	private JLabel labelNom = new JLabel(); //Création du titre nom
	...
	private JComboBox comboConcert = new JComboBox(concertTab); //création de la liste déroulante qui recupère le tableau
	private JRadioButton buttonPlaceDebout = new JRadioButton(); //création des boutons à cocher
	private JRadioButton buttonPlaceAssise = new JRadioButton();
	private ButtonGroup  groupTypePlace = new ButtonGroup(); //création du groupe qui recuperera les deux boutons

	Boolean verif = false;//Initialize the verification to false. It will help us to know if the user did not fill a field

	Box windowPanel = Box.createVerticalBox();//Création de la box qui accueillera les autres box

	Box panTitle = Box.createHorizontalBox(); //Pour le titre
	Box panNom = Box.createHorizontalBox(); //Pour le nom
	Box panPrenom = Box.createHorizontalBox(); //Pour le prénom
	...
	public void Display(Boolean b) {//C'est la fonction afficher qui recupere un Boolean (OUI OU NON)
		...
		if (b == true) {
			...
			verif = false;
			System.out.println("votre billet a �te bien r�server");
	...
	public Fenetre(){
		...
		//buttonPlaceAssise.setMnemonic(KeyEvent.VK_B);
		buttonPlaceAssise.setSelected(true);
		//buttonPlaceDebout.setMnemonic(KeyEvent.VK_B);
	...
	//Create one pan to gather every panel
